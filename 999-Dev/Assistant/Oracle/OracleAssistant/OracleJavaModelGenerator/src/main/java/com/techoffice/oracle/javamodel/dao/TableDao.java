package com.techoffice.oracle.javamodel.dao;
...
public class TableDao {
	...
	public List<Column> getTableColumnList(String tableName){
		MapSqlParameterSource namedParameters = new MapSqlParameterSource();
		namedParameters.addValue("TABLE_NAME", tableName);
		List<Column> list = namedParameterJdbcTemplate.query("SELECT COLUMN_NAME, DATA_TYPE, DATA_LENGTH FROM ALL_TAB_COLS WHERE TABLE_NAME = :TABLE_NAME", 
			namedParameters, new RowMapper<Column>(){
				public Column mapRow(ResultSet rs, int rowNum) throws SQLException {
					Column column = new Column();
					column.setColumnName(rs.getString("COLUMN_NAME"));
